...
_Last updated: 2026-07-07 (two-phase visual-prompts bundle (ADR-0013) — `check_phase_state` kills the refs-only false green, `check_ref_image_path` field lint, gate 1R, and three honest 🔵 Human rows (scene->space completeness with the declined under-segmentation lint, framing-to-real-pixels, late-ref-edit backstop); human checkpoints now number three)._
_Prior: 2026-07-05 (canon/style bundle — eye-glow lint closes the last original Gap (ADR-0010), style-suffix variant family (ADR-0009), Final-mix AV sync declared as Human)._
...
| **Gate 1R** - references approved (two-phase episodes) | 🟢 Machine (linkage) + 🔵 Human (the judgement) | `approvals.json` gate `"1R"` record consumed by `pipeline_integrity.py` (`phase2_asserted` bool computed from disk, passed into `gate_findings`) ([ADR 0013](adr/0013-two-phase-visual-prompts.md)) | Episodes `>= TWO_PHASE_FROM_EP` (10): once Phase-2 scenes are asserted (visual prompts on disk, Phase-1 sentinel gone), a gate-1R record OR an artifact-pinned 1R waiver must exist, else FAIL. EP01-09 are exempt (documented pre-two-phase cutover). Honest limit: 1R attests a human approved the ref PROMPTS; the ref IMAGES live in gitignored `raw/` and are not machine-verifiable — preventive at the skill layer, detective at CI, same posture as gates 1/2. EP10 v01 carries the interim artifact-pinned waiver until the v02 two-phase run records the real 1R. |
...
| Reference approval (gate 1R) | After Phase-1 reference authoring, before Phase-2 scene prompts (two-phase episodes, EP10 onward) |
