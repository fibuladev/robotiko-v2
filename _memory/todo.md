...
**End card:** 6 finished 1920×1080 PNGs in `episode-10/05_video/raw/`. Round 1 (A/B/C: single art line + URL) judged by Fibula as too cryptic for strangers — right call. Round 2 adds the explanation tier (poem on top / plain-English meaning line / URL). Round 2b (Fibula's wording): middle lines reframed to **"one film told in ten episodes"** — signals character continuity + long-form film in one phrase, replacing "ten films" (read as ten separate videos). Current set: `endcard_D.png` (*I came to walk beside.* + "One film, ten episodes — every prompt and decision behind it is public."), `endcard_E.png` (THIS SERIES IS OPEN SOURCE. + "Story, prompts, pipeline — the complete making of one film told in ten episodes." — prefer 5s card), `endcard_F.png` (poem + full framing sentence — needs 5s card). **RESOLVED (2026-08-05): Fibula picked E** — "the headline opens the door correctly; whoever walks in finds the poem, the story and the engineering on their own." Card = `endcard_E.png`, 5s (4:30–4:35, rides the gong decay; **export = 275s**). Guide §8 + QA locked to E; A/B/C/D/F stay in raw/ as archive (D = runner-up).
...
## 2026-08-18 Session Summary (Pre-Public Line-by-Line Audit - Part 3, episodes)

**Task:** The same human-grade read applied to `episode-01/` through `episode-10/` - every tracked file, ten read-only audit agents, one per episode. This closes the full-tree pre-public read. Every BLOCKER was re-verified by hand before it reached a fix agent, and one of them did not survive that check.

**The finding that mattered most:** `episode-09/02_music/ep09_musical_metadata.json` still carried "the wound is where the light enters" as *Mevlana's teaching*. The 2026-07-06 attribution sweep established that line is Barks-era apocrypha and replaced it in master.md with the authentic Divan-i Kebir ruin/treasure image - but this JSON was the last surviving instance in the tree. It is the exact packaging master.md's own argument criticizes, so a primed reader could have caught the project doing the thing it objects to. Now carries the authentic image.

**The gate that could not see the violation:** `forbidden_terms_gate.py` scopes to `episode-*/03_direction/*.md` - the same directory the 2026-07-10 public-doc sweep covered. Run it directly on `episode-03/04_visuals/ep03_visual_prompts_v01.md` and it FAILS on a banned term at line 555; run `--full` and it PASSES, because that file is out of scope. The sweep and the gate meant to protect it inherited one blind spot. Two sibling blind spots found with it: `capcut_guide_validator.py` silently skips EP03 (its header regex expects `| Shot |`, EP03's table starts `| # | Shot |`), and the visual-prompt and hygiene lints only ever open the latest version per episode, so EP02's motion script v01 (0 of 35 prompts carry the video suffix) and EP06's visual prompts v01 are unchecked by every gate. Term fixed; widening the scopes is deliberately deferred so it does not surface new failures on launch day.

**A template sentinel that made the repo lie about its own gate:** `ep05_motion_script_v01.md` kept the scaffold line "This template is auto-populated by Claude", which `pipeline_integrity.py` reads as proof of an unfilled file - so the checker printed "[ ] Step 6: Motion Script (pending) <- Human approval gate 2" for a launched episode whose gate-2 approval is sha-pinned. Removing it brought the file into the hygiene lint's scope for the first time and exposed pre-existing non-ASCII debt in its prompt blocks, normalized with the repo's own `--fix`.

**Also fixed:** the only HTML TODO in the tree, sitting inside EP04's paste-ready YouTube description (now the live playlist URL); EP02's visual-prompt header and self-validation checklist, which claimed 47 prompts and six keyframe pairs against an actual 39 and four, and referenced two scenes (S19b, S28b) that do not exist; EP03's S30 Scene Context note, which denied the episode's own canon on its most sensitive scene and reintroduced religious-practice vocabulary the direction files had been swept of, now phrased as the generator-facing framing instruction it was always meant to be; a real poet's name ASCII-stripped in EP06's launch copy (the only such instance in the repo, against the repo's own standard); 30 clip blocks in EP08's motion script whose missing blank lines made GitHub swallow the Camera Move field, plus three concatenated bullets; and a local drive label in EP10's sync-QC record.

**One agent finding that verification killed:** EP03's CapCut guide was reported as 22 seconds short of its music. The Timestamp column is in fact contiguous 0:00 -> 524s; the Scene Dur column simply floors each row, so summing it gives 502s. The guide's total was right all along. A clarifying line now says so, to stop the next reader "fixing" a timeline that is not broken. Adversarial verification earned its place here - acting on that report would have damaged a correct edit list.

**State:** 10 fixes across 10 files, three gate-2 sha pins re-baselined (EP03, EP05, EP08 motion scripts), suite 12/12 green, all 22 approval pins matching disk. Deferred by decision to after T0: the validator scope widening above, and the cross-cutting cosmetic classes (unchecked gate-2 approval boxes in five launched episodes, stale 4K export specs, "open source after the finale" copy, half-applied eye-glow legacy notes).
