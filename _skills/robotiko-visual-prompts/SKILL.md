...
- **Text Prompt:** The full standalone reference prompt — wide establishing shot, no characters (for environments), full spatial detail, ending with the mandatory suffix. **The marker MUST stay exactly the two words `Text Prompt:` wrapped in double asterisks (bold), spelled and cased that way.** Do not rename, restyle, or re-case it. The ADR-0006 prompt-hygiene lint scopes REF prompts by that exact marker; renaming it silently drops every reference prompt out of hygiene enforcement (ASCII/no-em-dash, forbidden aesthetics, suffix presence).
...
- [ ] Each REF block carries Design Brief + Environment Geometry (generation spec) + Narrative anchors + canonical Reference Image Path + a Text-Prompt marker spelled exactly — the two words `Text Prompt:` in bold (1.4)
...
- [ ] **No TEMPLATE_MARKERS anywhere in v02** (`{XX}`, `auto-populated by Claude`, `Do not fill manually`, `[Claude generates`, `Shot X: ...`) — once the sentinel is removed, ANY such marker makes the validator silently scaffold-skip the whole file (every scene goes unvalidated = false green). Write concrete filenames (`ep10_s01_selected.png`), never `{XX}`. Also: **inside the generated episode file**, the bolded `Text Prompt:` marker may appear only as a real field label above a real prompt — never in prose, notes or checklists, where the prompt-extractor would match it as a genuine (suffix-less) prompt and fail the file. When you need to mention it there, write "the Text-Prompt marker" unbolded. (This constraint is scoped to `ep{XX}_visual_prompts_v{VV}.md`, which the lint globs; it does not apply to this skill's own field definitions in 1.4.)
