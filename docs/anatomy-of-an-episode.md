...
- **Human gates.** Direction is committed at two points — **after dramaturgy** and **after the motion script** — and the machine refuses to proceed past either without explicit approval. From EP10 onward a third human checkpoint sits inside the visual stage: the reference gate (gate 1R), where the human approves the generated reference images as real pixels before any scene prompt is written. The gates sit precisely where a mistake gets exponentially more expensive to undo (story → 49 clips; world → every scene framed against it; plan → render credits).
