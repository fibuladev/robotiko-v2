# EP03 — YouTube Package

> Episode: EP03 — "Anatolian Trials" (THE CRUCIBLE)
> Generated: 2026-05-12
> Updated: 2026-05-22
> Skill: robotiko-youtube-packager v4.0
> Standards: `_management/youtube_metadata_standards.md`

---

## 1. VIDEO TITLE

```
They Folded Him Like Cloth | ROBOTIKO v2.0 EP03 | Cinematic AI Series
```

Characters: 69 / 80 max ✅

---

## 2. VIDEO DESCRIPTION

```
The test comes in the mother tongue. (Bu bölüm tamamen Türkçe'dir.)
ROBOTIKO v2.0 — Episode 03 of 10. A cinematic AI sci-fi series.
▶ Start from Episode 01: [playlist link]

BÖLÜM 3: ANADOLU'NUN TESTİ
Bir Chrome Android (Robotiko v2.0), spiritüel aydınlanma umuduyla Anadolu
sokaklarında yürür. Ancak bu topraklar anlatıldığı gibi değildir; gelenin
niyetini tartar, gidenin yükünü ölçer. Serttir, düzdür ve beklentileri affetmez.
Bu, kötülüğün değil, yanlış ayarlanmış bir "yazılım" beklentisinin hikayesidir.

(In English: EPISODE 3: ANATOLIA'S TEST — A Chrome Android (Robotiko v2.0) walks
the streets of Anatolia hoping for spiritual enlightenment. But these lands are
not as they are described; they weigh the intent of the one who arrives and
measure the burden of the one who leaves. They are harsh, plain, and unforgiving
of expectations. This is not a story of evil, but of a mis-configured "software"
expectation.)

—

THE LORE

Note: After the spoken prologue, the verses follow the traditional
8-syllable meter of Turkish folk poetry.

Prolog:

Bu topraklar...
Anlatıldığı gibi değildir.
Her gelen aydınlanmaz burada.
Her giden arınmış sayılmaz.
Anadolu sabır dağıtmaz.
Dayanıklılığı yoklar.
Kimi kendini yolcu sanır.
Oysa bu yollar
terazi gibidir.
Gelenin niyetini tartar.
Gidenin yükünü ölçer.
Burada hayat
güzel sözle ikna olmaz.
Serttir! Düzdür!
Çoğu zaman sessizdir.
Bazıları bu sessizliği
hikmet sanır.
Bazılarıysa
ölçüldüğünü fark etmeden eksilir.

Bu,
kötülüğün değil.
Yanlış ayarlanmış
bir beklentinin
hikâyesidir.

— Yükseltmedim Modelini, Hazırladım Ecelini —

Rûmî dedi anda dondu,
Apar topar bilet buldu,
İşte yeni mekân buydu,
Tüttürmeye ocağını.

Gitme dedim, dinlemedi,
Erken dedim, anlamadı,
Tehlikeyi tınlamadı,
Açtı hâr'a kucağını.

Yükseltmedim modelini,
Hazırladım ecelini.

Varış günü ramazandı,
Açtı, midesi kazındı,
O son lokma pek hazindi,
Yerken meydan dayağını.

Güç bela doğuya kaçtı,
Bir aşiret dehşet saçtı,
Ama o sevgiye açtı,
Öptü mermi yanağını.

Yükseltmedim modelini,
Hazırladım ecelini.

Kuzeye sürdü yolunu,
Karıştırdı sağ solunu,
Maganda kırdı kolunu,
Hem sapladı bıçağını.

Güney sıcak, her yer tuzak,
Hanutçular açtı kucak,
Sandı şefkat duyulacak,
Çaktı Felek çakmağını!

Yükseltmedim modelini,
Hazırladım ecelini.

Biri dedi güven bana,
Abin keyif verir sana,
Götürdü nayt klabına,
Sundu adisyon dağını.

Ah bilemedi sonunu,
Aldılar metâl donunu,
Verdi Stiftung fonunu,
Yedi ayva yaprağını!

Yükseltmedim modelini,
Hazırladım ecelini.

Halüsinasyonlar gördü,
Kader de ağını ördü,
Bir cinci hocaya vardı,
Düzeltmeye odağını.

Hem okundu hem üflendi,
Metâl gövde de ellendi,
Korkuları tetiklendi,
Topladı tas tarağını!

Yükseltmedim modelini,
Hazırladım ecelini.

Mecburi yönü batıydı,
İstanbul zirve katıydı,
Kıldan ince sıratıydı,
Kaydırdılar ayağını.

Kâbuslardan hortladılar,
Metrobüste fortladılar,
Örtü gibi katladılar,
İndirdiler sancağını!

Yükseltmedim modelini,
Hazırladım ecelini.

Fibulam nedir bu hâller,
Mutlu mu sol-liberaller,
Güneş selamlayan eller,
Kazdı onun toprağını.

—

A human wrote the story as lyrics, shaped the musical direction, designed the story arc,
and built a tech-art pipeline.
Music: Suno. Images: Nano Banana. Video: Kling + Seedance. Pipeline: Claude. Edit: CapCut.
The full production pipeline will be open source after the finale.

ROBOTIKO v2.0 — A 10-episode CyberAnatolian cinematic series.

Previous: The Tech Guru's Downfall → [EP02 URL]
Next: The Moon Has No Light of Its Own → [EP04 URL]

#aiscifi #cinematicai #robotiko #aifilm #aiseries #scifi #aifilmmaking #klingai
```

**Checklist:**
- [x] Opens with AI categorization signal (3-line format) ✅
- [x] Episode hook from youtube_metadata_standards.md (exact text) ✅
- [x] Playlist CTA above the fold ✅
- [x] "THE LORE" label above poem (not "Lyrics:" — avoids music classification signal) ✅
- [x] 8-syllable meter note added (Turkish folk poetry tradition) ✅
- [x] Musical structure markers removed (pure poetry) ✅
- [x] Turkish synopsis added (BÖLÜM 3: ANADOLU'NUN TESTİ) ✅
- [x] "(Bu bölüm tamamen Türkçe'dir.)" inline with hook ✅
- [x] "Prolog:" label separates free-form intro from 8-syllable verses ✅
- [x] Refrain as destan title: "— Yükseltmedim Modelini, Hazırladım Ecelini —" ✅
- [x] AABB quatrain punctuation: commas within, period at stanza end (no ellipsis, no dash) ✅
- [x] Chorus included at each appearance (repetition is thematic weight) ✅
- [x] Instrumental breaks removed (poetry, not lyric sheet) ✅
- [x] Cross-links present (prev + next) ✅
- [x] NO timestamps/chapters ✅
- [x] NO "About ROBOTIKO" block ✅
- [x] #aiart included ✅
- [x] No spoilers for future episodes ✅
- [x] No genre hashtags (#progrock, #psychedelicrock, etc.) ✅
- [x] "anatolian pop rock" removed (genre tag) ✅
- [x] Turkish subject tags added (yapay zeka filmi, kısa film, etc.) ✅
- [x] No inspiration credit (none specified for EP03 in youtube_metadata_standards.md) ✅

---

## 3. PINNED COMMENT

```
This episode exists in Turkish for a reason.
```

Pin immediately after video goes live. No additions, no emojis. Exact text from youtube_metadata_standards.md.

---

## 4. TAGS

**Base tags (constant across all episodes — per youtube_metadata_standards.md):**
```
cinematic ai series, ai sci-fi series, ai short film, ai animated series, sci-fi short film, ai filmmaking, ai generated movie, ai film, ai animation, ai storytelling, dystopian sci-fi, android story, sci-fi animation, kling ai, suno ai, veo ai, ai video generation, robotiko, robotiko v2, ai art, generative ai, sci-fi series 2026
```

**EP03-specific tags (English):**
```
anatolian trials, the crucible, turkish language, anatolia, istanbul, metrobus
```

**EP03-specific tags (Turkish — subject tags, not genre):**
```
yapay zeka filmi, robot hikayesi, kısa film, yapay zeka sanatı, türkçe kısa film, sci-fi türkçe
```

**Combined (paste into YouTube):**
```
cinematic ai series, ai sci-fi series, ai short film, ai animated series, sci-fi short film, ai filmmaking, ai generated movie, ai film, ai animation, ai storytelling, dystopian sci-fi, android story, sci-fi animation, kling ai, suno ai, veo ai, ai video generation, robotiko, robotiko v2, ai art, generative ai, sci-fi series 2026, anatolian trials, the crucible, turkish language, anatolia, istanbul, metrobus, yapay zeka filmi, robot hikayesi, kısa film, yapay zeka sanatı, türkçe kısa film, sci-fi türkçe
```

Characters: ~430 / 500 max ✅

**TAG STRATEGY (all episodes):**
- ✅ Film-first approach per `_management/youtube_metadata_standards.md` Section 5
- ✅ Genre tags REMOVED (progressive rock, psychedelic rock, etc.)
- ✅ AI-generated tags ADDED for correct categorization

---

## 5. THUMBNAIL GUIDANCE

**Primary option — Metrobus Compression:**
- **Scene:** S32 — Metrobus (Robotiko physically folded by crowd, claustrophobic fluorescent lighting)
- **Why:** Most visually striking and unique moment. Chrome android crushed in everyday Istanbul commute = visual contradiction. High contrast (fluorescent white vs. chrome metal).

**Alternative — Anatolian Landscape:**
- **Scene:** S08 — Arrival (Chrome Robotiko standing in dusty Anatolian town, warm earth tones vs. cold chrome)
- **Why:** Establishes the episode's core visual contrast (metal vs. earth). Brighter, more thumbnail-friendly.

**Rules:**
- **"03" only** — bottom-left corner, small, italic white serif font. No other text.
- **Face/character must be visible at mobile thumbnail size**
- **Brightness:** Ensure adequate contrast for YouTube grid visibility
- **Progression note:** EP01 = cosmic wonder, EP02 = corporate hubris, EP03 = ground-level survival. Visual arc continues downward.

---

## 6. END SCREEN

Per `youtube_metadata_standards.md` Section 10: **No end screens EP01-EP09.**

- **Previous episode:** EP02 — "The Tech Guru's Downfall" → [EP02 URL]
- **Next episode:** EP04 — [EP04 Title] → [EP04 URL when available]
- **Playlist:** Add to "ROBOTIKO v2.0 — Complete Cinematic AI Sci-Fi Series" playlist

---

## 7. ADDITIONAL SETTINGS

- **Category:** Film & Animation
- **Language:** Turkish
- **Altered content disclosure:** Yes
- **Comments:** Allow all (moderation on)
- **Subtitles:** None (intentional — Turkish-only creates mystery and engagement for international viewers; encourages comments, rewatching, community translation)
- **Visibility:** Scheduled for [DATE] at 20:00 CET
- **Export reminder:** 16:9 canvas from CapCut (NOT 2.35:1)

---

## 8. EP03 INSTAGRAM STRATEGY — TURKISH MARKET

EP03 is the only Turkish-language episode. This is a one-time opportunity to reach Turkish audiences on Instagram.

### Hashtag Strategy (3-Tier)

**Tier 1 — High-volume Turkish discovery (keşfet entry):**
```
#kesfet #kesfetteyiz #türkiye #istanbul #anadolu
```

**Tier 2 — Mid-volume niche (right audience):**
```
#yapayzeka #aiart #kisafilm #animasyon #scifi #bilimkurgu #robotik #dijitalsanat #yapayzekafilmi
```

**Tier 3 — Low-volume specific (community):**
```
#türkkısafilm #anadolurock #metrobüs #istanbulhayatı
```

**Combined EP03 Instagram hashtags (paste — replaces standard set for this episode only):**
```
#robotiko #cyberanatolian #conceptalbum #aiart #aimusic #genai #kesfet #kesfetteyiz #türkiye #istanbul #anadolu #yapayzeka #kisafilm #animasyon #scifi #bilimkurgu #dijitalsanat #yapayzekafilmi #türkkısafilm #metrobüs
```

### Instagram Posting Rules (EP03 Only)

- **Caption language:** Turkish (Instagram's language detection routes to Turkish feeds)
- **Location tag:** İstanbul (Turkish keşfet is location-weighted)
- **Posting time:** 19:00–21:00 Turkey time (UTC+3) — Turkish peak engagement
- **Note:** Standard episodes use English captions + 18:00 CEST. EP03 is the exception.

### Why This Is EP03-Only

Other 9 episodes are in English. This Turkish strategy does NOT carry forward. When "Atomize EP03 for social" runs, apply these rules to all EP03 clips instead of the standard English hashtag/caption set.

---

*"Would Fibula approve this?"*
