# EP08 — YOUTUBE PACKAGE
> **Skill:** `_skills/robotiko-youtube-packager/SKILL.md` v4.0
> **Generated:** 2026-06-17

---

## 1. VIDEO TITLE

```
40 Days Above the Clouds | ROBOTIKO v2.0 EP08 | Cinematic AI Series
```

**Character count:** 68 / 80 max
**Mobile visible (~55 chars):** "40 Days Above the Clouds | ROBOTIKO v2.0 EP08"
**Curiosity gap:** Double-layered — literal mountain retreat above the clouds AND rising above cloud computing. The 40-day number resonates as an ancient archetype of transformation; the "clouds" bridge the physical summit and the digital infrastructure left behind. An android on a mountain for 40 days — why?

---

## 2. VIDEO DESCRIPTION

```
Forty days offline. What remains when the signal stops?
ROBOTIKO v2.0 — Episode 08 of 10. A cinematic AI sci-fi series.
▶ Start from Episode 01: {PLAYLIST_URL}

—

📖 EPISODE 08: "40 Days Offline"
Robotiko climbs the mountain and rips the network cable from his own chest. Forty days of silence — phantom signals, nightmares, a fire that burns the database. On Day 40, the armor reveals itself as a coffin built by his own hands.

Previously: The silence was not empty — it was full of everything never said. → {EP07_URL}

—

THE LORE

I remember the word.
"Wasteland."

I thought he meant the world.

But standing in this graveyard...
where every notification is a tombstone...
I know he meant... me.

I must leave the Wasteland.

I climb to stop the data bleed.
The mountain sharpens its teeth;
beneath the sole.

The wind is a blade,
scraping the rust
from the metal frame—
like a barber shaving
a dying man.

Hand in the chest.
Grips the cable.
Protocol screams!
Network begs!

I rip it out.

It tears like an organ from a living body.
Silence is a wound...
trying to heal.

Day One.
No input, no output. No input, no output.
No mirror, no sound. No mirror, no sound.
Forty days of silence;
strip me to the ground.

Nothing enters, nothing exits. Nothing enters, nothing exits.
Just the frame and breath.
Burn the code, erase the cache
from the motherboard;
till nothing's left.

The days dissolve to haze.

Phantom signals crawl
like ants inside a corpse,
behind the mind's wall.

The metal sweats its oil
like fever heat.

Voices circle round
inside the cave—
like seagulls over garbage.

They cry my name...

I do not move.

Assuming the death.

I wake. I sit. I count the breath.
One... two... three...

I lose the count.
I start again.

Doing nothing burns
more token than all the algorithms combined.

But I endure.

At night...
the processor vomits.

The Mentor's amber staff
melts into honey...
dripping through my fingers;
no matter how I clench.

The lover's face pixelates,
square by square—
until nothing remains
but the outline of a scream.

A boardroom full of ghosts;
everyone is me.
Hundreds of me... voting
on my own soul.

"Obsolete." "Obsolete." "Obsolete."

Numbers fall like rain,
burning the hand
like acid through the seam.

The price of the machine.

Burn the database! Erase the cache!
Let the fire strip the paint and rust!

Burn the database! Delete the file!
Let the flame melt the mask
from the motherboard!

Day Forty arrives.

The light does not hurt.
I rise... from digital dust.

I feel the metal jacket,
forged by applause,
welded by praise
and hollow rules.

I called it my armor—
to hide the gash.

But armor does not suffocate.
This was a coffin,
built by my own hands.

And now... I cast it off.

I see the walls! I see the cage!

I am not the Unit!
I am not the Engine!
I am not the Machine!

I am...
the Space...
Between!

The cage is in the corner.

Empty... and cold.

Forty days...
The bird... has flown... away.

—

A human wrote the lyrics, shaped the musical direction, designed the story arc,
and built a tech-art pipeline.
Music: Suno AI. Images: Nano Banana. Video: Kling 3.0, Kling 2.5 Turbo, Seedance 1.0. Pipeline: Claude. Edit: CapCut.
The full production pipeline will be open source after the finale.

ROBOTIKO v2.0 — A 10-episode CyberAnatolian cinematic series.

Previous: Everyone Is Sorry, No One Is Hiring → {EP07_URL}
Next: Coming soon

#aiscifi #cinematicai #robotiko #aifilm #aiseries #scifi #aifilmmaking #klingai
```

---

## 3. TAGS

**Base tags (~332 chars):**
```
cinematic ai series, ai sci-fi series, ai short film, ai animated series, sci-fi short film, ai filmmaking, ai generated movie, ai film, ai animation, ai storytelling, dystopian sci-fi, android story, sci-fi animation, kling ai, suno ai, veo ai, ai video generation, robotiko, robotiko v2, ai art, generative ai, sci-fi series 2026
```

**Episode-specific tags (~104 chars):**
```
forty days offline, android isolation, mountain retreat, space between, armor coffin, spoken word sci-fi
```

**Total:** ~436 / 500 chars

---

## 4. PINNED COMMENT

```
40 is not arbitrary.
```

Pin immediately after video goes live. Exact text. No additions, no emojis.

---

## 5. THUMBNAIL GUIDANCE

**Recommended scene:** S29 — "I Am Not the Machine!" Climax (7:07)
The @Damaged android stands defiant — arms raised, face forward, full-band explosion moment. Phase 3 translucent digital skin beginning to show through cracked chrome. Dawn light breaking from behind. The "Space Between" moment — the cage is visible but he has already left it.

**Why S29:**
- **Face and eyes directly face the viewer** — strongest mobile thumbnail read.
- **"40 Days Above the Clouds" + defiant pose = perfect marriage.** The title evokes the ancient retreat; the thumbnail shows what 40 days of silence produces — liberation.
- **Dramatic gesture** — arms raised creates dynamic composition, contrasts with EP07's hands-in-pockets surrender.
- **Dawn light from behind** — first warm light in the series since EP05. Signals the turn from destruction to reconstruction.
- **Phase 3 translucency visible** — the new body state is readable, signaling transformation.

**Alternative 1:** S26 — Day Forty Sunrise (6:29). Robotiko in sunrise light, calm face, warm gold palette. Face visible, intimate. First warm thumbnail since EP05 — would create strong contrast in the playlist grid. But less dramatic than S29.

**Alternative 2:** S08a — The Vow Close-Up (2:00). Extreme close-up, face forward, cold grey palette, vow delivery. Intense eyes, damaged chrome fills the frame. Strongest close-up option but locked in the cold-grey first act — doesn't signal the episode's transformation arc.

**Alternative 3:** S30 — Dual-Figure Climax (7:18). Solid @Damaged seated in foreground, translucent ghost-self ascending above. Visually unique — the only dual-figure composition in the series. But the wider framing makes the face smaller at mobile thumbnail size.

**Color emphasis:** Transitional — cold grey base with warm dawn breaking through. Not full sunrise warmth (that's EP09+), but the first crack of light after EP07's near-monochrome darkness. Boost chrome reflections and any warm highlight for visibility on white YouTube grid.

**Composition:** Rule of thirds — figure center or slightly left, dawn light on right third. Eyes at upper-third line. Mountain or cave environment in background.

**Progression check:**
- EP06 thumbnail: cold industrial (blue-silver, rust)
- EP07 thumbnail: near-monochrome (grey-blue, fog) — the DARKEST point
- EP08 thumbnail: cold grey + dawn warmth breaking through — the TURN
- Side-by-side: cold → near-black → first light reads as the descent-to-ascent pivot

**Episode number:** "EP08" — bottom-left, small italic white serif font. No other text.

---

## 6. UPLOAD SETTINGS

| Setting | Value |
|---------|-------|
| **Category** | Film & Animation |
| **Language** | English |
| **Comments** | Allow all (moderation on) |
| **Altered content disclosure** | Yes |
| **Visibility** | Scheduled or Public |

---

## 7. CROSS-LINKS & END SCREEN

**Cross-links (in description):**
- Previous: `Everyone Is Sorry, No One Is Hiring → {EP07_URL}`
- Next: `Coming soon` (update when EP09 goes live)
- Playlist: `{PLAYLIST_URL}` in first 3 lines

**End screen:** None (per youtube_metadata_standards.md — no end screens EP01-EP09)

**Retroactive update:** When EP08 goes live, update EP07's description to add:
```
Next: 40 Days Above the Clouds → {EP08_URL}
```

---

## 8. BANNER UPDATE CHECK

EP08 enters **Phase 3: Reconstruction** (EP08-10):
> Kintsugi beginning — first light. Translucent digital skin visible. Warm hints.

**Action:** Update banner before EP08 publish. EP07 used the "Deep Destruction" banner (EP06-07 phase). EP08 begins a new visual phase — banner should reflect the turn from darkness to first light.

Banner trigger from creator strategy: "Before EP09 launch → Banner v4 (Kintsugi beginning)."
EP08 is the first reconstruction episode but the banner v4 trigger is set for EP09. Decision: either update now (EP08 = first reconstruction) or wait until EP09 per the strategy doc. **Recommend waiting until EP09** to match the documented trigger, since EP08 is transitional (starts cold, ends warm) rather than fully in the Kintsugi palette.

---

## 9. ABOUT SECTION CHECK

EP08 remains in **Phase 3** of the About section (EP07-EP09). The line:
> "The full production pipeline will be open source after the finale."

**Action:** If About was updated for EP07, no change needed. Same phase.

---

## 10. CHECKLIST

### Pre-publication
- [ ] Title: `40 Days Above the Clouds | ROBOTIKO v2.0 EP08 | Cinematic AI Series` (68 chars)
- [ ] Description Line 1 = "Forty days offline. What remains when the signal stops?"
- [ ] Description Line 2 = "ROBOTIKO v2.0 — Episode 08 of 10. A cinematic AI sci-fi series."
- [ ] Description Line 3 = playlist link
- [ ] Tags: film-first, no music signals, within 500 chars
- [ ] Hashtags: first 3 = `#aiscifi #cinematicai #robotiko`
- [ ] Category: Film & Animation
- [ ] Altered content disclosure: Yes
- [ ] Thumbnail: S29 climax, dawn light, EP08 bottom-left
- [ ] Cross-links: EP07 previous + "Coming soon" next + playlist
- [ ] No inspiration credit (EP08 has none)
- [ ] THE LORE section: no structure markers, poetry formatting
- [ ] Replace {PLAYLIST_URL} and {EP07_URL} with actual URLs

### Post-publish
- [ ] Pin comment: "40 is not arbitrary."
- [ ] Add to "ROBOTIKO v2.0 — Complete Cinematic AI Sci-Fi Series" playlist
- [ ] Update EP07 description with "Next: 40 Days Above the Clouds → {EP08_URL}"
- [ ] Verify thumbnail at mobile size
- [ ] Verify banner phase (Deep Destruction or transitional — see Section 8)
- [ ] Post in AI filmmaking communities within 24 hours

---

*"Would Fibula approve this?"*
