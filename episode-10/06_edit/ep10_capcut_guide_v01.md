...
### 1. `34b.mp4` — different tool, 720p, and the only clip carrying audio

S34b (The Beckon) was **not** generated by Kling. Kling could not execute the gesture — it read the
open-palm beckon as presenter-style arm movement instead of the small, human *come* the shot is built on.
The keeper was generated with **Google (Veo / Gemini)**, which got the gesture right. **The clip stays.**
The gesture *is* the shot: this is the last frame of ten episodes, and a sharp clip with the wrong
choreography is a failure where a soft clip with the right one is not.

| | `34.mp4` (S34a) | `34b.mp4` (S34b) |
|---|---|---|
| Tool | Kling 3.0 | **Google (Veo / Gemini)** |
| Resolution | 1920×1080 | **1280×720** |
| Video bitrate | 11.5 Mbps | **1.9 Mbps** |
| Audio track | none | **AAC 128 kbps — present** |

Three consequences, each with a fix:

**a) ⚠️ MUTE OR DETACH THE AUDIO ON IMPORT.** `34b.mp4` is the **only** clip in the episode with an audio
stream — the generator's synthetic ambience. Left live, it plays underneath the album's final gong. This
is the one item here that would actually damage the ending. Right-click → Detach/Mute immediately on import.

**b) The sharpness step at the 4:23 cut.** S34a → S34b is a static-to-static cut on the *same*
composition, so a change in apparent sharpness is maximally visible — nothing else in the frame moves to
distract from it. **Match the pair to each other, not to the episode:** take a few percent of sharpness
off `34.mp4` (or nudge its grain up) so the two sit level. The eye detects the *step*, not the absolute
softness. Do this on the pair only — do not soften the rest of the episode to meet it.

**c) Banding risk from the low bitrate.** 1.9 Mbps is thin for the smooth morning-gold gradients in the
meadow and sky. It survives because the shot is nearly static (H.264 spends its bits well when nothing
moves), and because the global film grain is applied at 1080p *after* the upscale — grain is the correct
dither for banding. **Push S34b's grain toward the top of the 10–15% band** as a per-clip override.

> Optional pre-step: rather than letting CapCut scale a 1.9 Mbps source at runtime, upscale offline once
> with a proper filter and a fat bitrate, then import that — `ffmpeg -i 34b.mp4 -vf scale=1920:1080:flags=lanczos
> -c:v libx264 -crf 16 -an 34b_1080p.mp4`. Marginal gain, but free, and the `-an` also solves (a).
...
- [ ] ⚠️ 34b.mp4  S34b  10.01s  (**Google Veo / Gemini** — not Kling) — **720p + HAS AUDIO, see Pre-Flight §1**
...
**Two per-clip overrides (technical, not stylistic):**

| Clip | Shot | Override | Why |
|------|------|----------|-----|
| 34b.mp4 | S34b | Grain toward the **top** of the 10–15% band | Dithers the low-bitrate banding in the meadow/sky gradients (Pre-Flight §1c) |
| 34.mp4 | S34a | **−2–3% sharpness** (or a matching grain nudge) | Levels the pair so the 4:23 cut shows no quality step (Pre-Flight §1b) |
...
- **Tool-specific corrections:**
  ...
  - **34b.mp4 comes from a different generator entirely** (Google Veo / Gemini) and will not respond to the
    LUT the way its Kling neighbour does — expect a different contrast curve and skin/metal warmth. Grade it
...
- [ ] **`34b.mp4` audio detached or muted** — it is the only clip with an audio stream; nothing may sound under the final gong
- [ ] S34a/S34b pair matched to each other: no sharpness step and no color step at the 4:23 cut
- [ ] S34b grain pushed toward 15% — no banding in the meadow/sky gradients
